...
def blrObjFunction(params, *args):
    ...
    p = np.reshape(p, (716, 1))

    #calculate Y
    Y = sigmoid(np.dot(train_data, p))

    #error function
    #a = np.multiply(labeli, np.log(Y))
    #b = np.multiply((1.0 - labeli), np.log(1.0 - Y))
    d = np.multiply(labeli, np.log(Y))+np.multiply((1.0 - labeli), np.log(1.0 - Y))
    error = np.sum(d)
    error = -error

    t = np.multiply((Y - labeli) , train_data)
    error_grad = np.sum(t, axis=0)
...
print('\n Training set Accuracy: ' + str(am.score(train_data, train_label)*100) + '%')
print('\n Validation set Accuracy: ' + str(am.score(validation_data, validation_label)*100) + '%')
print('\n Testing set Accuracy: ' + str(am.score(test_data, test_label)*100) + '%')
